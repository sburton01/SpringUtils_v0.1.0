...
<cfcomponent name="RemoteUserService" 
			displayname="RemoteUserService:RemoteProxyBean" 
		...
		<cflock name="RemoteProxyBean.#variables.proxyId#.Setup" type="readonly" timeout="5">
			<cfif not StructKeyExists(variables, "constructed") or not variables.constructed>
				...
				<cftry>		
					...
					<cfset remoteFactory = bf.getBean("&remoteUserService") />
					<cfset variables.target = bf.getBean("remoteUserService") />
	...
	<cffunction name="getUser" access="remote" returntype="any" output="false" > 
<cfset var rtn = callMethod('getUser', arguments) />
